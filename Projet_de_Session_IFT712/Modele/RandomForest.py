...
class RandomForest(StrategieClassification):
    ...
    def afficher(self, x_train, t_train, x_test, t_test, feature_names=None, class_names=None):
        ...
        # Affiche l'arbre de décision du premier estimateur du modèle Random Forest
        # (Note: Cette partie est commentée car le code génère une figure mais ne l'affiche pas.
        # Vous pouvez décommenter ces lignes si vous souhaitez sauvegarder l'arbre sous forme d'image.)
        #fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(4, 4), dpi=800)
        #tree.plot_tree(self.random_forest_model.estimators_[0],
        #               feature_names=feature_names,
        #               class_names=class_names,
